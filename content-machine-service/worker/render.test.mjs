...
const t=String(Math.floor(Date.now()/1000)),raw='{"id":"e1"}',secret='test-key';const sig=createHmac('sha256',secret).update(t+'.'+raw).digest('hex');assert.equal(verifyCallback(raw,{'x-timestamp':t,'x-signature':sig},secret),true);assert.equal(verifyCallback(raw+'tampered',{'x-timestamp':t,'x-signature':sig},secret),false);
