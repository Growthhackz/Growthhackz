...
export class RpcError extends Error { constructor(message: string, public code?: number, public data?: unknown, public timeout = false) { super(message); } }
...
type RawSimulation = { err: unknown; logs?: string[]; unitsConsumed?: number; accounts?: (RawAccount | null)[] };
const decode = (a: RawAccount | null): Account | null => a ? { data: new Uint8Array(Buffer.from(a.data[0], 'base64')), owner: a.owner, lamports: a.lamports } : null;
  ...
  const client = {
    ...
    async getAccounts(keys: PublicKey[]) {
      ...
      for (let i = 0; i < keys.length; i += 100) {
        const r = await call<{ value: (RawAccount | null)[] }>('getMultipleAccounts', [keys.slice(i, i + 100).map(String), { encoding: 'base64', commitment: 'confirmed' }]);
        out.push(...r.value.map(decode));
      }
      return out;
    },
    async getProgramAccounts(program: PublicKey, filters: object[]) {
      const r = await call<{ pubkey: string; account: RawAccount }[]>('getProgramAccounts', [program.toBase58(), { encoding: 'base64', commitment: 'confirmed', filters }], 15000);
    ...
    async simulate(transaction: string, accounts: PublicKey[] = []): Promise<Simulation> {
      const r = await call<{ value: RawSimulation }>('simulateTransaction', [transaction, { encoding: 'base64', sigVerify: false, replaceRecentBlockhash: true, commitment: 'confirmed', ...(accounts.length ? { accounts: { encoding: 'base64', addresses: accounts.map(String) } } : {}) }], 15000);
